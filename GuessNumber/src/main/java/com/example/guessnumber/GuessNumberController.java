...
public class GuessNumberController {
    ...
    Notice that this field name "lblOutputMsg"
     is identical with its corresponding id value in the FXML file:
     <Label fx:id="lblOutputMsg"/>
     */
    @FXML
    ...
    This field is also named "txtUserNumber" as its corresponding one in the FXML file:
    ...
    /*
        ...
        outside button click event method as we don't want to generate a random number
        for every click! only when we start the application:
     */
    Random rand = new Random();
    /*
    Using .nextInt(1,11) to generate a random number between 1 as minimum and  up to 10 as maximum value
         ...
         try {
             ...
             if (userNumber>=1 && userNumber<=10) {
                 if (userNumber == randNumber)
                     msg="Good Job!, The number is " + randNumber;
